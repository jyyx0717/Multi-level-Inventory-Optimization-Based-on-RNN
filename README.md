...
该项目通过构建供应链的“数字孪生”模型，利用自动微分技术计算总成本对库存水位的梯度，从而通过梯度下降算法（SGD）直接求解复杂约束下的最优库存策略。
...
2.  **核心优化 (Optimization / Solve)**
    ...
    *   **功能**：加载网络模型，从零库存或指定初始值开始，使用 SGD 算法寻找最低成本的库存策略。
...
2.  **收敛曲线 (`cost_curves.png`)**:
    *   **Total Cost (蓝线)**: 总体目标函数值，呈下降趋势。
    *   **Holding Cost (橙线)**: 随着优化进行，为了避免缺货，持有成本通常会上升，最终与缺货风险达成平衡。

3.  **评估报告 (`service_level_summary.csv`)**:
    *   `CSL`: 周期服务水平（不缺货周期的百分比）。
    *   `FR`: 需求满足率。
